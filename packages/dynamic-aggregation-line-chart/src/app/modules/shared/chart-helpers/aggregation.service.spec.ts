    ...
    describe('AGGREGATION_CANDIDATES', () => {
        it('is sorted in strictly ascending order of seconds', () => {
            for (let i = 1; i < AGGREGATION_CANDIDATES.length; i++) {
                expect(AGGREGATION_CANDIDATES[i].seconds)
                    .withContext(`index ${i}: "${AGGREGATION_CANDIDATES[i].value}" should come after "${AGGREGATION_CANDIDATES[i - 1].value}"`)
                    .toBeGreaterThan(AGGREGATION_CANDIDATES[i - 1].seconds);
            }
        });
        ...
        it('contains all expected day variants', () => {
            ...
            for (const v of ['1d', '2d', '3d', '4d', '5d', '6d']) {
                expect(values).withContext(`missing ${v}`).toContain(v);
            }
        });
        ...
        it('all values match the Cumulocity format: 1–3 digit integer + single-letter unit', () => {
            ...
            for (const { value } of AGGREGATION_CANDIDATES) {
                expect(value).withContext(value).toMatch(PATTERN);
            }
        });

        it('all values are between 2 and 4 characters (per API spec)', () => {
            for (const { value } of AGGREGATION_CANDIDATES) {
                expect(value.length).withContext(value).toBeGreaterThanOrEqual(2);
                expect(value.length).withContext(value).toBeLessThanOrEqual(4);
            }
        });
    });
