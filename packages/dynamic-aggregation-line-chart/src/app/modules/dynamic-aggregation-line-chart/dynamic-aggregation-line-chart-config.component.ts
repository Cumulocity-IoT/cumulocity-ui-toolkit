...
import { DynamicAggregationLineChartConfig, ThresholdConfig } from './ps-line-chart.model';
...
export class DynamicAggregationLineChartConfigComponent
  implements DynamicComponent, OnBeforeSave, OnInit
{
  ...
  readonly aggregationOptions = ['min', 'max', 'avg', 'sum', 'count', 'stdDevPop', 'stdDevSamp'];
  ...
  onAssetSelected(event: { items?: IManagedObject | IManagedObject[] | null } | null): void {
    const item = Array.isArray(event?.items)
      ? event.items[0]
      : (event?.items ?? (null as IManagedObject | null));
  ...
  readonly filteredDatapoints = computed(() =>
    this.datapoints().filter((dp) => dp.__active !== false)
  );
  ...
  onBeforeSave(
    config?: DynamicAggregationLineChartConfig
