...
import { ManagedObjectUpdateService } from './managed-object-update.service';
...
export class SimulatorSettingsService {
  ...
  generateRequest() {
    ...
    for (const value of this.measurementService.uniqueMeasurementsArray) {
      for (const { temp } of this.helperService
        ...
        const toBePushed = this.instructionService.instructionToCommand(instruction);
        const index = String(this.allInstructionsArray.length - 1);
        const toBePushedWithIndex: IndexedCommandQueueEntry = {
          ...toBePushed,
          index: index,
  ...
  createIndexedCommandQueue(
    ...
    const indexedCommandQueue: IndexedCommandQueueEntry[] = [];
  ...
  removeIndicesFromIndexedCommandQueueArray(
    ...
    const commandQueueWithoutIndices = [];
  ...
  buttonHandler(
    ...
    if (inputField.name === 'sleepsEqualToInstructions') {
      ...
      for (const entry of allInstructionsSeries) {
        if (entry.steps) {
          steps += +entry.steps + 1;
        }
  ...
  pushToResultTemplate(instruction: Instruction) {
    const toBePushed: CommandQueueEntry = this.instructionService.instructionToCommand(instruction);
    const index = String(this.allInstructionsArray.length - 1);
    const toBePushedWithIndex: IndexedCommandQueueEntry = {
      ...toBePushed,
      index: index,
  ...
  updateCommandQueueAndIndicesFromIndexedCommandQueue(
    ...
    const commandQueue = this.removeIndicesFromIndexedCommandQueueArray(indexedCommandQueue);

    this.updateAll(indexedCommandQueue, commandQueue);
    this.setIndexedCommandQueueUpdate();

    const additionals: AdditionalParameter[] = this.indexedCommandQueue.map((entry) => {
      return {
        index: entry.index,
  ...
  objectContainsSearchString(series, searchString) {
    ...
    const value = pickBy(modifiedSeries, (value, key) => {
  ...
  modifyInstructionSeries(series) {
    let modifiedSeries =
      series.type === 'SmartRest' ? { ...series.instruction, type: series.type } : series;
