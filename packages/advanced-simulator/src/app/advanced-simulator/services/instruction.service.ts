...
import { CommandQueueEntry, CommandQueueType, MessageIds } from '../models/command-queue.model';
...
export class InstructionService {
  ...
  private alarmService = inject(AlarmsService);
  ...
  smartRestInstructionToCommand(
    ...
    if (smartRestSelectedConfig) {
      ...
      const customValuesFromInstruction = Object.keys(instruction).filter(
        (key) => key !== 'type' && key !== ''
      );
      ...
      const filteredIndex = customPaths.findIndex(
        (customPathsForEntry) =>
          this.compareArrays2(customValuesFromInstruction, customPathsForEntry) === true
      );
    ...
    if (selectedSmartRest.smartRestFields.mandatoryValues.length) {
      values = [''];
    } else {
      values = [];
    }
  ...
  commandQueueEntryToInstruction(commandQueueEntry: CommandQueueEntry): Instruction {
    ...
    if (commandQueueEntry.type === CommandQueueType.message) {
      const smartInstruction = { type: InstructionCategory.SmartRest };
  ...
  createSmartRestDynamicForm(instructionValue) {
    const smartRestForm: InputField[] = [];
