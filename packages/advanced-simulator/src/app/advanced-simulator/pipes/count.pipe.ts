...
export class CountPipe implements PipeTransform {
  transform(instructionSeries: SeriesInstruction[]): {
    alarms: number;
    events: number;
    measurements: number;
    sleep: number;
    ...
    stats.events =
      instructionSeries.filter((entry) => entry.type == 'BasicEvent').length +
      instructionSeries.filter((entry) => entry.type == 'LocationUpdateEvent').length;
