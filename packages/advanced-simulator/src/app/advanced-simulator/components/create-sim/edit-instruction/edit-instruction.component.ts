...
export class EditInstructionComponent implements OnInit {
  ...
  private smartService = inject(SmartRESTService);
  ...
  addSingleSmartRestInstruction() {
    ...
    const indexedCommandQueueEntry = { ...smartRestCommandQueueEntry, index: 'single' };
  ...
  addOrUpdateInstruction(index: number) {
    const instructionValue: Instruction | Instruction2 = {};
    ...
    const commandQueueEntry = this.instructionService.instructionToCommand(
      instructionValue as Instruction
    );

    if (this.displayAddView && this.selectedEditView !== 'SmartRest') {
      const indexedCommandQueueEntry = { ...commandQueueEntry, index: 'single' };

      if (this.commandQueueEntryIndex) {
        this.indexedCommandQueue.splice(
          this.commandQueueEntryIndex + 1,
          0,
      ...
      if (this.edit) {
        const idx = this.edit.index;
        const indexed = { ...commandQueueEntry, index: idx } as IndexedCommandQueueEntry;
  ...
  @Input() set editedValue(value: CommandQueueEntry) {
    if (value) {
      ...
      const instruction: Instruction =
        this.instructionService.commandQueueEntryToInstruction(value);
      ...
      if (this.instructionValue.type === InstructionCategory.SmartRest) {
        ...
        this.smartRestForm = this.instructionService.createSmartRestDynamicForm(
          this.instructionValue
        );
