...
import { ICurrentTenant, IResult } from '@c8y/client';
...
export class CreateSimComponent implements OnInit {
  ...
  private instructionsService = inject(InstructionService);
  ...
  @HostListener('window:scroll', ['$event']) onScroll(event) {
    const checkSimulator = document.getElementById('check-simulator');
    const bulkSimulator = document.getElementById('bulk-simulator');
    const maintainSimulator = document.getElementById('maintain-simulator');
    const scrollValue = window.scrollY + window.innerHeight * this.scrollTransitionInPercentage;
  ...
  ngOnInit() {
    ...
    const additionals: AdditionalParameter[] = this.mo.c8y_additionals;
  ...
  delete(value) {
    ...
    const index = this.allInstructionsSeries.indexOf(value);
  ...
  autoScrollTo(newStep: number) {
    ...
    const y = document.getElementById(element).offsetTop;
