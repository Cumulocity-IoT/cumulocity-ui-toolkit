  ...
  describe('serializeNode', () => {
    ...
    it('serializes a numeric comparison without quotes', () => {
      const root: BuilderNode = {
        kind: 'and',
        children: [
          { kind: 'comparison', field: 'count', operator: 'gt', valueType: 'number', value: '0' },
        ],
      };
    ...
    it('serializes null comparisons', () => {
      const root: BuilderNode = {
        kind: 'and',
        children: [
          { kind: 'comparison', field: 'owner', operator: 'eq', valueType: 'null', value: '' },
        ],
      };
    ...
    it('serializes has / hasany / bygroupid / isinhierarchyof', () => {
      expect(
        serializeNode({ kind: 'and', children: [{ kind: 'has', fragment: 'c8y_IsDevice' }] })
      ).toBe('has(c8y_IsDevice)');
      expect(
        serializeNode({ kind: 'and', children: [{ kind: 'hasany', fragments: 'a, b' }] })
      ).toBe('hasany(a,b)');
      expect(serializeNode({ kind: 'and', children: [{ kind: 'bygroupid', ids: '10300' }] })).toBe(
        'bygroupid(10300)'
      );
      expect(
