...
describe('MeasurementInterceptorService.tryParseParams', () => {
  ...
  it('returns null when the URL does not end with measurement/measurements', () => {
    expect(
      parse(service, makeReq('https://example.com/measurement/measurements/series', validParams()))
    ).toBeNull();
  });
  ...
  it('returns null when source is missing', () => {
    ...
    expect(
      parse(service, makeReq('https://example.com/measurement/measurements', rest))
    ).toBeNull();
  });
  ...
  it('returns null when pageSize is below the minimum', () => {
    expect(
      parse(
        service,
        makeReq('https://example.com/measurement/measurements', {
          ...validParams(),
  ...
  it('returns parsed source, dateFrom, dateTo as Date objects', () => {
    const result = parse(
      service,
      makeReq('https://example.com/measurement/measurements', validParams())
    );
