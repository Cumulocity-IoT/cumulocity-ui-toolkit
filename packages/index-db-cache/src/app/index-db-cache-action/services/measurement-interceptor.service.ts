...
import { MIN_HISTORICAL_MS, clampCoverageTo } from './interceptor-helpers';
 ...
 * - `dateFrom` is at least {@link MIN_HISTORICAL_MS} in the past; coverage is
 *   additionally clamped to `now - MIN_HISTORICAL_MS` so the live edge of the
 *   window is refetched on the next request
 *
...
export class MeasurementInterceptorService implements HttpInterceptor {
  ...
  private async handleWithCache(
    ...
    // 3. All gaps were fetched completely — commit to cache. Coverage is
    //    clamped away from the live edge so late-arriving (or backdated)
    //    measurements near `now` are picked up on the next request.
    for (const { gap, measurements } of gapResults) {
      await this.cache.storeRange(
        ...
        fragmentSeries,
        gap.from,
        clampCoverageTo(gap.from, gap.to),
        measurements
      );
    }
